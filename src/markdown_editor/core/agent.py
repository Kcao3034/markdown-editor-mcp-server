...
class DocumentAgent:
    ...
    def __init__(self, config: Config, document_path: Optional[str] = None):
        ...
        self.engine = DocumentEngine(
            ...
            journal_path=config.document.journal_path
        )
        ...
        system_prompt = config.prompts.get("system_prompt", "You are a document editing agent.")
    ...
    def _tool_insert_after(self, path: str, content: str, element_type: str = "paragraph") -> dict:
        result = self.engine.insert_after(path, content, element_type)
        return result.to_dict()
    
    def _tool_insert_before(self, path: str, content: str, element_type: str = "paragraph") -> dict:
