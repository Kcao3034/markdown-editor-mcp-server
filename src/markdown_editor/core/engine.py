...
class DocumentEngine:
    ...
    def insert_after(self, path: str, content: str, element_type: str = "paragraph") -> OperationResult:
        """Inserts element after specified path."""
        return self._apply_operation(
            OperationType.INSERT_AFTER, path, content, 
            extra={"element_type": element_type}
        )
    
    def insert_before(self, path: str, content: str, element_type: str = "paragraph") -> OperationResult:
        """Inserts element before specified path."""
        return self._apply_operation(
            OperationType.INSERT_BEFORE, path, content,
            extra={"element_type": element_type}
        )
    ...
    def _apply_operation(
        ...
        try:
            if op_type == OperationType.REPLACE:
                ...
                # Form insertion
                elem_type = extra.get("element_type", "paragraph") if extra else "paragraph"
                ...
                if op_type == OperationType.INSERT_AFTER:
                    ...
                    if insert_pos < len(working_content) and working_content[insert_pos] != '\n':
                        insert_text = '\n' + insert_text
                else:
