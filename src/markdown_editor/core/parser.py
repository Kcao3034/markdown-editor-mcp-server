        ...
        while i < len(lines):
            ...
            if line.startswith('>'):
                ...
                while i < len(lines) and (lines[i].startswith('>') or lines[i].strip() == ''):
                    if lines[i].strip() == '' and quote_lines and not quote_lines[-1].startswith('>'):
                        break
    ...
    def _assign_paths(self, element: Element, parent_path: str = ""):
        ...
        for child in element.children:
            ...
            if child.type == ElementType.HEADING:
                ...
                child.path = child.content if not parent_path else f"{parent_path} > {child.content}"
            else:
                ...
                child.path = path_name if not parent_path else f"{parent_path} > {path_name}"
