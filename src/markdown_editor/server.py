    ...
    return [
        ...
        Tool(
            ...
            outputSchema={
                ...
                "properties": {
                    ...
                    "before": {"type": "object", "description": "Element before target"},
        ...
        Tool(
            ...
            outputSchema={
                "type": "object",
                "properties": {
                    "element": {"type": "object", "description": "The requested element"},
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "path": {
                        "type": "string",
                        "examples": ["Introduction > paragraph 1", "Conclusion", "Features > list 2"]
                    },
                    "new_content": {
                        "type": "string",
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "element_type": {
                        "type": "string",
                        "enum": ["heading", "paragraph", "list", "code_block", "blockquote"],
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "path": {
                        "type": "string",
                        "examples": ["Introduction > paragraph 3", "Old Section", "Deprecated > list 1"]
                    }
                },
                "required": ["file_path", "path"],
                "additionalProperties": False
        ...
        Tool(
            ...
            outputSchema={
                ...
                "properties": {
                    "success": {"type": "boolean"},
                    "reverted_count": {"type": "integer", "description": "Number of operations reverted"}
                }
            }
        ),
...
async def call_tool(name: str, arguments: dict) -> Any:
    ...
    try:
        ...
        if name == "search_tools":
            ...
            relevant = [t.name for t in all_tools if query in t.name or (t.description and query in t.description.lower())]
            return {
                "content": [TextContent(type="text", text=f"Relevant tools: {', '.join(relevant)}")],
        ...
        if name == "list_directory":
            ...
            return {"content": [TextContent(type="text", text=str(items))], "structuredContent": {"items": items}, "isError": items is None}

        elif name == "create_file":
            res = await create_file(arguments["path"], arguments.get("content", ""))
            return {"content": [TextContent(type="text", text="File created")], "structuredContent": res, "isError": "error" in res}

        elif name == "create_directory":
            res = await create_directory(arguments["path"])
            return {"content": [TextContent(type="text", text="Directory created")], "structuredContent": res, "isError": "error" in res}

        elif name == "delete_item":
            res = await delete_item(arguments["path"])
            return {"content": [TextContent(type="text", text="Item deleted")], "structuredContent": res, "isError": "error" in res}

        # 3. EDIT OPS (require file_path)
        file_path = arguments.get("file_path")
        if not file_path:
            return {"content": [TextContent(type="text", text="Missing file_path")], "isError": True}

        if name == "get_document_structure":
            res = await get_document_structure(file_path, arguments.get("depth", 2))
            return {"content": [TextContent(type="text", text="Structure extracted")], "structuredContent": {"structure": res}, "isError": False}

        elif name == "search_text":
            res = await search_in_document(file_path, arguments["query"])
            return {"content": [TextContent(type="text", text=f"Found {len(res)} matches")], "structuredContent": {"results": res}, "isError": False}

        elif name == "get_context":
            res = await get_element_context(file_path, arguments["path"])
            return {"content": [TextContent(type="text", text="Context extracted")], "structuredContent": res, "isError": "error" in res}

        elif name == "read_element":
            res = await read_element(file_path, arguments["path"])
            return {"content": [TextContent(type="text", text="Element read")], "structuredContent": res, "isError": "error" in res}

        elif name == "replace_content":
            res = await replace_content(file_path, arguments["path"], arguments["new_content"])
            return {"content": [TextContent(type="text", text="Content replaced")], "structuredContent": res, "isError": "error" in res}

        elif name == "insert_element":
            res = await insert_element(file_path, arguments["path"], arguments["element_type"], arguments["content"], arguments.get("where", "after"), arguments.get("heading_level", 1))
            return {"content": [TextContent(type="text", text="Element inserted")], "structuredContent": res, "isError": "error" in res}

        elif name == "move_element":
            res = await move_document_element(file_path, arguments["source_path"], arguments["target_path"], arguments.get("where", "after"))
            return {"content": [TextContent(type="text", text="Element moved")], "structuredContent": res, "isError": "error" in res}

        elif name == "delete_element":
            res = await delete_element(file_path, arguments["path"])
            return {"content": [TextContent(type="text", text="Element deleted")], "structuredContent": res, "isError": "error" in res}

        elif name == "update_metadata":
            res = await update_document_metadata(file_path, arguments["metadata"])
            return {"content": [TextContent(type="text", text="Metadata updated")], "structuredContent": res, "isError": "error" in res}

        elif name == "undo":
            res = await undo_changes(file_path, arguments.get("count", 1))
            return {"content": [TextContent(type="text", text="Undo performed")], "structuredContent": res, "isError": "error" in res}

        return {"content": [TextContent(type="text", text=f"Unknown tool: {name}")], "isError": True}
...
async def main():
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        ...
        await app.run(
            read_stream,
            write_stream,
            app.create_initialization_options(notification_options=notification_options)
        )
